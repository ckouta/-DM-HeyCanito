...
function detalle(id) {
  ...
  app.request.get('http://localhost:3005/producto/'+id,{}, function (data) {
    console.log("ya no esta pasando por aqui");
    var valores = JSON.parse(data);
    valores.data.forEach(element => {
      ...
      td.push('     </div>');
      td.push('     <div class="col">');
      td.push('         <button class="col button button-fill color-orange">Carrito ver</button>');
      td.push('     </div>');
      td.push('  </div>');
    });
    console.log(td.join(''));
...
/* <div class="data-table card">
            <table>
              <thead>
                ...
                <th class="label-cell">categoria</th>
                <th class="label-cell">imagen</th>
                <th class="label-cell">cantidad de personas</th>
              </thead>
              <tbody id="articles">
